...
def count_tensor_elements(x):
  ...
  if len(shape_is) == 1:
    print(f"total elemnts of tensor is = {num_elements}")
  
  else:
    num_elements = 1
    for element in shape_is:
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x_view = x.reshape(2, 3, 4) # baraye sakhtane dota goroohe 12 taii

  x_permute = x_view.permute(1,0,2)

  y = x_permute.reshape(3,8)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  min_args = torch.argmin(x , dim=1)
  row_indices = torch.arange(x.shape[0])

  y[row_indices , min_args] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop == True:
    ...
    for batch in range(x.shape[0]):
      mul = torch.matmul(x[batch] , y[batch])
      result.append(mul)
...
def normalize_columns(x):
  ...
  std = (((x - mu) ** 2).sum(dim=0) / (rows - 1)) ** 0.5
